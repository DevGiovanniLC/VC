...
class DataLoader:
    def __init__(self):
        self.data_intelligence = pd.read_csv("Trabajo\\data\\dog_intelligence.csv", index_col=['Breed'])
        self.data_atributtes = pd.read_csv("Trabajo\\data\\dog_attributes.csv", index_col=['Breed'])
    ...
    def get_attributes(self, item):
        ...
        intelligence = search.get_attribute("obey").get_attribute("Classification").get()
        height_range_cm = search.get_attribute("height_low_inches").get_attribute("height_high_inches").get()
        height_range_cm = [ float(x) * 2.54 for x in height_range_cm ]
        
        weight_range_kg = search.get_attribute("weight_low_lbs").get_attribute("weight_high_lbs").get()
        weight_range_kg = [ float(x) * 0.45359237 for x in weight_range_kg ]
        
        
        return (intelligence, str(RangeType(height_range_cm, "cm")), str(RangeType(weight_range_kg, "kg")))
